...
def extract_squares(warped):
	...
	for horizontal in range(8):
		for vertical in range(8):
			images.append(cv2.getRectSubPix(warped,(int(l/8),int(w/8)),((l/16)+horizontal*(l/8),(w/16)+vertical*(w/8))))
...
for x in range(64):
	...
	plt.subplot(8,8,x+1)
	plt.imshow(list_of_images[x])
fig.axes.get_xaxis().set_visible(False)
fig.axes.get_yaxis().set_visible(False)
plt.show()

# cv2.imshow('dst',list_of_images[7])
# if cv2.waitKey(0) & 0xff == 27:
#     cv2.destroyAllWindows()
# for image in list_of_images:
# 	cv2.imshow('dst',image)
# 	raw_input("Press key for next image")
# 	cv2.destroyAllWindows()