...
def send_move(move,board,team):
	...
	##type 3: promotion
	##type 4: en-passant
	if (board.is_capture(move[0]) and (not board.is_en_passant(move[0])) and (len(str(move[0]))==4)):
		move_type=1*2
	...
	elif (board.is_capture(move[0]) and (not board.is_en_passant(move[0])) and (len(str(move[0]))==5)):
		move_type=4*2

	elif (len(str(move[0]))==5):
		move_type=5*2
	...
	if (team==0):
		if (move_type/2==0):
			...
			row2=row_dict_lower_0[user_in[2]]
			move2=row2+1000*column2
			move3=row_dict_higher_0[user_in[2]]+1000*column_dict_higher_0[user_in[3]]
		...
		if (move_type/2==0):
			...
			row2=row_dict_lower_1[user_in[2]]
			move2=row2+1000*column2
			move3=row_dict_higher_1[user_in[2]]+1000*column_dict_higher_1[user_in[3]]
...
def is_promotion(board, colour,location):
	for x in board.pieces(1,not colour):
		if (location==x):
			return True
...
difficulty=int(raw_input("Enter difficulty: "))
	...
	while(True):
		if(colour==1): ##Player plays black
			...
			while (board.result()=='*'): #Checks if game is over
				...
				if (difficulty>0):
					move=engine.go(movetime=100,depth=difficulty) #Engine makes first move
				else:
					move=engine.go(movetime=100)
				...
				if(player_move[3]=='1'):
					if(is_promotion(board, colour, fun.square_to_num(player_move[0:2]))):
						player_move=player_move+'q'
